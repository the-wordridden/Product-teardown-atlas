...
| **Status** | **Passes 1, 1b, 1c complete. Gate assessment in `05-stage-1-gate.md`.** See §7 |
| **Sources verified (opened and inspected)** | **42** (21 in pass 1; 12 in pass 1b; 9 in pass 1c) |
| **Sources attempted and blocked** | 7 (§6); X006 retried and still blocked |
...
### Pass 1c — switching costs, loop mechanisms, Business Model terms, first selection account (2026-09-22)

Sources **STR-S034** to **STR-S042**. Findings:

- **Switching cost has a documented shape, and Link is the sharp edge.** Stripe's own export page says "We believe our customers own the sensitive data they entrust to Stripe… If you decide to leave Stripe for another payment processor, we'll work with your new processor's team to securely transfer your credit card data" — to any PCI DSS Level 1 processor [S037]. The same page: "You can't transfer the payment credentials saved with Link between payment processors. We exclude any credentials saved through Link from our exports." Stripe also "doesn't export your account's payment history, subscriptions, or other objects". A merchant whose volume runs through Link (Lovable: 58% [S041]) therefore has a vault that is majority non-portable. This is the first moat mechanism in the register with a documented component D (consequence) — though the consequence is stated by Stripe, not yet observed in a migration account.
- **Loop mechanism with a documented transition.** A Standard connected account "is a conventional Stripe account where the connected account has a direct relationship with Stripe, is able to log in to the Dashboard, and can process charges on their own" [S038]. Platforms such as Shopify onboard sellers as Standard accounts; the seller is then a Stripe account holder in their own right. The transition platform → seller → direct Stripe user is structurally evidenced. Its rate is not.
- **Capital is a data product.** Eligibility "based on their Stripe payment activity", "at least 3 months of payments on Stripe"; the lender is Celtic Bank, Lead Bank, YouLend or Fundbox by country; Stripe "acts as an intermediary and technology provider" [S040]. Stripe's payment data underwrites a loan someone else books.
- **First independent selection account.** Apify chose Braintree because "it was the only major global provider supporting European companies at the time", moved when "Stripe caught up and expanded its capabilities", and cites redundancy; 10,000 users, 9,000 cards, 2,000 PayPal accounts migrated in 18 working days [S042].
...
| **STR-S041** | Press release: "Lovable turns to Stripe to accelerate its growth" | 2026-01-14 | USE (customer self-report via Stripe), INT | "58% of Lovable's payment volume is driven by Link"; payments from "more than 150 countries"; "more than 125 local payment methods"; products: Billing, usage-based billing, Atlas, Payments, Link. CEO quote: "Stripe has been a partner from day one". Stripe-authored; treat as Stripe's characterisation of a customer. |
...
| **STR-S036** | docs.stripe.com/get-started/data-migrations | retrieved 2026-09-22 | CAP (AUTH) | Migration into Stripe: "Send the Stripe Migrations team details about your previous processor"; "we can only assist you if your request includes both customer records and the associated payment data"; three PAN flows: import, copy between Stripe accounts, export. |
| **STR-S037** | docs.stripe.com/get-started/data-migrations/pan-export | retrieved 2026-09-22 | **CAP (AUTH)**, moat component D (stated) | "We believe our customers own the sensitive data they entrust to Stripe"; "we can only transfer your card data to another PCI DSS Level 1-compliant payment processor"; "**You can't transfer the payment credentials saved with Link between payment processors. We exclude any credentials saved through Link from our exports.**"; "Stripe doesn't export your account's payment history, subscriptions, or other objects"; data remains accessible "as long as you don't close or delete your account". |
| **STR-S038** | docs.stripe.com/connect/accounts (legacy account types; page marked deprecated in favour of Accounts v2) | retrieved 2026-09-22 | **CAP (AUTH)**, PRC | Standard: "a conventional Stripe account where the connected account has a direct relationship with Stripe, is able to log in to the Dashboard, and can process charges on their own"; examples "store builders, such as Shopify". Express: "Stripe handles the onboarding and identity verification"; examples Airbnb, Lyft. Custom: "almost completely invisible to the account holder". Liability table: fraud and dispute liability "Connected account for direct charges, Platform for destination charges" (Standard); "Platform" (Express, Custom). "There's an additional cost for using Express or Custom connected accounts." "After you create a connected account, you can't change its type." |
| **STR-S039** | stripe.com/connect/pricing (US) | retrieved 2026-09-22 | **PRC (AUTH)** | "$2 per monthly active account"; "0.25% + 25¢ per payout sent"; "Starts at 2.9% + 30¢ per successful card charge"; instant payouts "1% of payout volume"; cross-border payouts "Starting at 0.25% of payout volume"; account debits "1.5% of debit volume"; 1099 e-file "$2.99"; "Stripe sets and collects processing fees from your users, so you don't have to." |
| **STR-S040** | docs.stripe.com/capital/how-capital-works (renders as "Stripe Capital for platforms") | retrieved 2026-09-22 | **CAP (AUTH)** | "Stripe prequalifies connected accounts daily using their payment activity on Stripe"; offer = "principal amount plus a flat fee, with no variable interest"; "Stripe withholds a fixed percentage of each transaction until the complete balance is repaid"; eligibility "At least 3 months of payments on Stripe… annual volume of at least 5,000 in local currency"; US providers "Celtic Bank or Lead Bank" (loans) and "YouLend" (MCA); UK "provided by YouLend"; AU "Fundbox"; DE/FR "YouLend"; "Stripe Technology Europe Limited acts as an intermediary and technology provider"; available AU, DE, FR, GB, US; funds "within 1–2 business days"; worked example principal 20,000 / premium 2,000 / rate 15%. |
...
| **STR-S034** | HN 40529033, "Stripe closed our account and refuses to migrate our data to another processor" (81 pts) | 2024-05-30 | USE (lived experience, SUGG), moat D (observed, one case) | Dating app, two years on Stripe, "all online dating services" added to Restricted Businesses "last year (without notice)"; "set to lose half our revenue in recurring Stripe subscriptions"; "0.05% dispute rate"; migration extension "recorded and approved" then payments disabled on the original date; "wasn't able to convince the support AI this was urgent enough". Counter-voice: "they shouldn't lose half their revenue if they need users to re-enter their credit card". Read against S037: the export path exists; this account is about execution and account status, not policy. |
| **STR-S035** | HN 34281730, "Show HN: List of Stripe Alternatives" (122 pts, 44 comments) | 2023-01-06 | CMP (INADM for position; SUGG for the competitive set as practitioners name it) | 24 named alternatives incl. Adyen, Braintree, Checkout.com, Mollie, Paddle, Square, PayPal; comment: "a more useful one… would be finding replacements for the specific Stripe services/APIs they are using. It's not as simple as saying 'just switch from Stripe to PayPal/Square'"; Paddle/FastSpring/Digital River identified as merchant-of-record "registration services", not processors. |
| **STR-S042** | Apify engineering blog, "Payment migration: Braintree to Stripe and beyond" (Jan Novotný) | 2025-02-13 | USE (selection account, T4/T5 hands-on), HIS | "we chose Braintree as our payment processor since it was the only major global provider supporting European companies at the time"; moved because "Stripe caught up and expanded its capabilities" and "we also wanted to build in redundancy"; "Braintree limits you to just two card exports"; "The whole process took 18 working days"; "more than 10,000 users, 9,000 payment cards, and 2,000 PayPal accounts". Runs both processors after migration. |
...
| STR-X006 | web.archive.org CDX and snapshots of stripe.com/pricing (2012–2024) | **"Internet Archive: Temporarily Offline"** on 2026-09-22, twice (passes 1b and 1c) | Retry before Stage 2 sign-off; not blocking |
...
## 7. What passes 1 to 1c did not do

- **Category B remaining:** Checkout/Payment Element, Treasury, Issuing, Atlas, Terminal, Tax docs. Not blocking: Product and Business Model can be drafted on what is open (payments pricing, Connect pricing and charge types, Link, Billing, Radar, Capital, data export).
- **No independent Link penetration figure.** 58% [S041] is one customer, reported by Stripe.
- **Selection evidence is one account** [S042] plus failure-mode threads [S033, S034]. Sufficient for a labelled Users section; thin for JTBD forces beyond "global coverage" and "redundancy".
